...
def accuracy_metric(idx_list, model, loss_fn, trainloader, validloader):
    if opt.data == 'cifar100':
        dm = torch.as_tensor(inversefed.consts.cifar10_mean, **setup)[:, None,
                                                                      None]
        ds = torch.as_tensor(inversefed.consts.cifar10_std, **setup)[:, None,
                                                                     None]
...
def reconstruct(idx, model, loss_fn, trainloader, validloader):
    ...
    for dis_iter in range(bin_num + 1):
        ...
        dw_loss = sum([
            cal_dis(dw_a, dw_b, metric=metric)
            for dw_a, dw_b in zip(fake_dw, input_gradient)
        ]) / len(input_gradient)

        dw_list.append(dw_loss)

    interval_distance = cal_dis(noise_input, ground_truth,
                                metric='L1') / bin_num
...
def main():
    ...
    if True:
        ...
        for attack_id, idx in enumerate(sample_list):
            ...
            print('attach {}th in {}, metric {}'.format(
                attack_id, opt.aug_list, metric))

    pathname = 'search/data_{}_arch_{}/{}'.format(opt.data, opt.arch,
                                                  opt.aug_list)
    ...
    for run in range(10):
        ...
        score = accuracy_metric(large_samle_list, model, loss_fn, trainloader,
                                validloader)
        score_list.append(score)

    print('time cost ', time.time() - start)

    pathname = 'accuracy/data_{}_arch_{}/{}'.format(opt.data, opt.arch,
                                                    opt.aug_list)
    root_dir = os.path.dirname(pathname)
    if not os.path.exists(root_dir):
        os.makedirs(root_dir)
