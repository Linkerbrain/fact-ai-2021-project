...
def get_data_loaders(train_batch_size, val_batch_size):
    fmnist = torchvision.datasets.FashionMNIST(download=True , train=True, root=".").train_data.float()

    data_transform = transforms.Compose([transforms.ToTensor(),
                                         transforms.Normalize((fmnist.mean()/255,), (fmnist.std()/255,))])

    train_set = torchvision.datasets.FashionMNIST(download=True, root=".", transform=data_transform, train=True)
    val_set = torchvision.datasets.FashionMNIST(download=True, root=".", transform=data_transform, train=False)
...
torch.save(model.state_dict(), "checkpoints/tiny_data_FashionMinist_arch_ResNet20-4/5.pth")
model = MnistResNet()
